...
<template>
  <aside class="flex h-screen" v-if="store.authUser && menus.length > 0">
    ...
    <div
      :class="!isClose ? 'translate-x-0 ease-out w-[30vw] md:w-[15vw] 2xl:w-[10vw]' : '-translate-x-1 ease-in w-full md:w-[9vw] 2xl:w-[4vw]'"
      class="fixed z-30 h-full overflow-auto inset-y-0 left-0 transition duration-300 transform overflow-y-auto lg:static lg:inset-x-0 hidden md:block">
