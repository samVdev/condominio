...
import type { expenseType } from "@/modules/Expenses/types/ExpenseType"
...
export default () => {
  ...
  const getExpenses = async () => {
    try {
      const response = await facturesUser.getExpensesFacture(`${new URLSearchParams(route.query as Params).toString()}`)
  ...
  const loadScroll = async (e: Event) => {
    ...
    if (scrollTop + clientHeight >= scrollHeight - 5) {
      ...
      const response = await facturesUser.getExpensesFacture(`${new URLSearchParams(route.query as Params).toString()}&offset=${data.offset}`);
