...
onMounted(() => getEarnings(`?offset=${earnings.offset}&${new URLSearchParams(route.query as Params).toString()}`))
...
<template>
  <main>
    ...
    <tablesHeader title="Ingresos" icon="money-bill-trend-up" :searchActive="true" @setSearch="({ e }) => setSearch(e)"
    ...
    <section className="relative mx-auto my-4 overflow-auto animate-fade-in">
      ...
      <div class="fakeTable overflow-hidden"
        ...
        @scroll="loadScroll">
        <article class="fakeTable-head" :class="toUserEarnings ? 'grid-cols-7' : 'grid-cols-8'">
          ...
          <p>Factura</p>
          <p v-if="!toUserEarnings">Acción</p>
        </article>

        <section v-if="earnings.rows.length > 0">
          <section>
            <div v-for="row in earnings.rows" :key="row.id" class="fakeTable-body"
              :class="toUserEarnings ? 'grid-cols-7' : 'grid-cols-8'">
              <p>{{ row.created }}</p>
              <p>{{ row.name }}</p>
              <p>{{ row.tower }}</p>
              ...
              <p v-if="!toUserEarnings">
                <ActionsTable v-if="!row.facture" :recibesBtn="false" :editBtn="true" :deleteBtn="true" 
                  @remove="deleteEarning(row.id)" @edit="$emit('form', { id: row.id })" />
              </p>
            </div>
          </section>
          <div class="fakeTable-body" v-if="earnings.Facture && earnings.Facture.USD > 0" :class="toUserEarnings ? 'grid-cols-7' : 'grid-cols-8'">
            <p></p>
            <p></p>
            <p></p>
            <p>{{parsePrices(earnings.Facture.USD).dol }}</p>
            <p>{{parsePrices(earnings.Facture.BS).bs }}</p>
            <p></p>
            <p></p>
            <p></p>
            <p v-if="!toUserEarnings">
